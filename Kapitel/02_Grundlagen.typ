...
Sumers et al. unterscheiden bei solchen Agenten interne Aktionen, etwa das Aktualisieren des Gedächtnisses, von externen Aktionen, mit denen der Agent auf seine Umgebung einwirkt @sumers2023cognitive. Erst die externen Aktionen gleichen die begrenzte Wissensbasis des Modells aus und setzen Schnittstellen zu Systemen außerhalb des Sprachmodells voraus.

== Werkzeugintegration in KI-Systemen

Die Anbindung externer Werkzeuge (_Tool Use_) setzt genau an den genannten Grenzen von #acrpl("LLM") an. Schick et al. zeigen mit _Toolformer_, dass ein Sprachmodell lernen kann, an geeigneten Stellen externe #acrs("API", plural: true) aufzurufen und deren Ergebnisse in die weitere Textgenerierung einzubeziehen @schick2023toolformer. Das Modell entscheidet dabei selbst, wann es welches Werkzeug mit welchen Argumenten verwendet.
