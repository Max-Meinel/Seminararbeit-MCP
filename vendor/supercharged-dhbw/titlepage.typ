...
#let titlepage(
  ...
  grid(
    ...
    if (head_of_study_program != none and head_of_study_program != "") {
    ...
    if (completion_period != none) {
      text(if (type(completion_period) == datetime) {
        completion_period.display(date-format)
      } else {
        completion_period.at(0).display(date-format) + [ -- ] + completion_period.at(1).display(date-format)
      })
    },
    ...
    if (submission_date != none) {
      text(submission_date.display(date-format))
    },
  )
}